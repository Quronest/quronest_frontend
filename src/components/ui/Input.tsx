...
const Input = ({
  ...
  return (
    <div
      className={clsx(
        "w-full h-13.5 px-5 rounded-xl overflow-hidden",
    ...
    >
      <label
        ...
        className={clsx(
          "absolute",
          "cursor-text text-neutral",
          "transition-all duration-200 ",
          placeholderClass,
          (isFocused || input) &&
            "text-sm -translate-y-3.5 bg-transparent text-primary origin-left",
        )}
      >
        {placeholder}
      </label>
      ...
      <input
        ...
        data-slot="input"
        className={clsx(
          "w-full mt-3.5 outline-none border-none bg-transparent",
        )}
        value={input}
      ...
      {type === "password" && (
        <button
          type="button"
          className={clsx(
            "text-neutral cursor-pointer absolute right-4",
            (isFocused || input) &&
