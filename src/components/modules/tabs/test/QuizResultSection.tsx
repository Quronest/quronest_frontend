...
export type TestDetailedResultProps = {
  quizSubmitResponseData: QuizSubmitResponseType;
  quizTask: DailyTaskType;
};
...
export const QuizResultSection = ({
  ...
  const totalQuestions = quizSubmitResponseData.total_questions;
  ...
  const correctCount = quizSubmitResponseData.total_correct;
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 select-none p-2 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border pb-4">
        <div className="space-y-1">
          ...
          <p className="text-neutral text-sm">
            {quizTask.domain || "Quiz"} • Review your answers and explanations
            below.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            size="md"
            onClick={() => console.log("user Requested retake")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-card border border-border p-6 rounded-2xl">
        ...
        <div className="flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-border pb-6 md:pb-0 md:pr-6">
          ...
          />
          <span className="text-neutral text-sm mt-3 font-medium">
            Your Final Score
          </span>
        </div>
        ...
        <div className="col-span-2 flex flex-col justify-center space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <span className="text-neutral text-xs uppercase font-bold tracking-wider">
                Status
              </span>
              <div>
                <span
            ...
            <div className="space-y-1">
              ...
              <p className="text-xl font-bold text-foreground">
                {formatMinutesSeconds(quizSubmitResponseData.total_time_spent)}
              </p>
            </div>
            <div className="space-y-1">
              <span className="text-neutral text-xs uppercase font-bold tracking-wider">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          {quizSubmitResponseData.questions.map(
            (question: QuizQuestionResultType, qIdx) => {
              const userAnswerOptId = question.user_answer_option_id;
                  ...
                  <div className="flex flex-col gap-3">
                    {question.options.map((option, optIdx) => {
                      const isCorrectOption =
                        option.id === question.solution.id;
                      const isUserSelected = option.id === userAnswerOptId;
                      ...
                      return (
                        <div
                          ...
                          className={clsx(
                            ...
                            !isCorrectOption &&
                              !isUserSelected &&
                              "border-border bg-card/20 text-neutral",
                          )}
                        >
                          {/* Mini Indicator icon */}
                          <div
                            className={clsx(
                              ...
                              !isCorrectOption &&
                                !isUserSelected &&
                                "border-neutral/30 bg-transparent",
                            )}
                          >
                            {isCorrectOption && (
                            ...
                            {isUserSelected && !isCorrectOption && (
                              <XCircle className="h-4 w-4 fill-red-500/10" />
                            )}
