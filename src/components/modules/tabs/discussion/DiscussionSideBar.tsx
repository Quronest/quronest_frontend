...
export const DiscussionSidebar = ({
  ...
  const { tabData, tab } = useTab();
  const { resourceId } = tabData as DiscussTabDataType;
  ...
  return (
    <aside
      className={clsx(
        "flex  flex-col border-r border-card-hover bg-card transition-all duration-300",
        collapsed ? "w-0 p-0 h-0 border-0!" : "w-72 h-full ",
      )}
    >
      <div
        className={clsx(
